-- ============================================================
-- SEEKOPS — Migración completa: seekops_legacy → seekops
-- Pasos: 2 (usuarios) → 3 (clientes) → 4 (proyectos)
--        → 5 (proyecciones de horas) → 6 (registros comerciales)
--
-- Requiere:
--   - dblink habilitado en PostgreSQL
--   - schema.sql + seeds.sql aplicados en seekops
--   - seekops_legacy accesible en el mismo servidor PostgreSQL
--
...
SELECT
  ...
  u.created::timestamp,
  'migration'
FROM dblink('dbname=seekops_legacy user=postgres',
...
LEFT JOIN (
  ...
  FROM dblink('dbname=seekops_legacy user=postgres','SELECT id, name FROM masters_team') AS old_t(id bigint, name varchar)
...
LEFT JOIN (
  ...
  FROM dblink('dbname=seekops_legacy user=postgres','SELECT id, name FROM masters_segmentation') AS old(id bigint, name varchar)
  JOIN client_segmentations n ON n.name = old.name
) cs ON cs.old_id = c.segmentation_id
LEFT JOIN (
  SELECT old.id AS old_id, n.id
  FROM dblink('dbname=seekops_legacy user=postgres','SELECT id, name FROM masters_sector') AS old(id bigint, name varchar)
...
LEFT JOIN (
  SELECT old.id AS old_id, n.id
  FROM dblink('dbname=seekops_legacy user=postgres','SELECT id, business_reason FROM projects_client') AS old(id bigint, business_reason varchar)
  JOIN clients n ON n.legal_name = old.business_reason
) c ON c.old_id = p.client_id
LEFT JOIN (
  SELECT old.id AS old_id, n.id
  FROM dblink('dbname=seekops_legacy user=postgres','SELECT id, email FROM users_user') AS old(id bigint, email varchar)
  JOIN users n ON n.email = old.email
) u ON u.old_id = p.manager_id
LEFT JOIN (
  SELECT old.id AS old_id, n.id
  FROM dblink('dbname=seekops_legacy user=postgres','SELECT id, name FROM masters_division') AS old(id bigint, name varchar)
  JOIN project_segmentation n ON n.name = old.name
) ps ON ps.old_id = p.division_id
LEFT JOIN (
  SELECT old.id AS old_id, n.id
  FROM dblink('dbname=seekops_legacy user=postgres','SELECT id, name FROM masters_layerproductivity') AS old(id bigint, name varchar)
