...
- **Given** estoy en la bandeja del gestor **When** la cargo **Then** veo una solicitud por cada línea pendiente (seeker × semana × proyecto, y por categoría en proyectos de área) de mis proyectos cuya aprobación está en PENDIENTE
...
- La unidad de aprobación es la **línea** (seeker × semana × proyecto, y por categoría en proyectos de área). Un gestor nunca puede aprobar horas de un proyecto que no gestiona.
- El estado de aprobación vive en `time_entry_approvals` (una fila por línea, `time_entry_line_id` UNIQUE). Ni `time_entries` ni `time_entry_lines` tienen columna `status`.
...
- **Given** estoy viendo una solicitud pendiente **When** hago clic en "Aprobar" **Then** se aprueba esa línea (sin afectar otras líneas/proyectos de la misma semana)

- **Given** confirmo la aprobación **When** hago clic en "Sí, aprobar" **Then** la aprobación de esa línea cambia a APROBADO y la solicitud desaparece de mi bandeja pendiente
...
- **Given** he aprobado todas las líneas de un proyecto en una semana **When** el seeker visualiza esa semana **Then** cada proyecto muestra su estado "Aprobado" (no existe un badge de estado a nivel semana)

### Supuestos y riesgos

- La aprobación es por línea, usando `linea_id` en el body del request.
...
- La observación aplica a la línea indicada (`linea_id`), no a toda la semana ni a todo el proyecto.
- Al observar, el gestor sugiere horas/extras: estas van a `suggested_hours`/`suggested_extra_hours` en `time_entry_approvals`. Las horas cargadas por el seeker (`time_entry_lines.hours`) nunca se modifican. Las horas efectivas de una línea APROBADO_CON_OBSERVACION son las sugeridas.
...
- **Given** horas de un proyecto fueron rechazadas **When** el seeker vuelve a cargar ese proyecto para esa semana **Then** puede crear una nueva línea PENDIENTE — la rechazada queda como histórico (línea `is_active=true`, su aprobación en RECHAZADO)

- **Given** el seeker intenta cargar un proyecto que ya tiene una línea activa con aprobación PENDIENTE o APROBADO **When** intenta enviar **Then** recibe error de duplicado y no puede enviarlo

### Supuestos y riesgos

- El rechazo es definitivo; la línea rechazada NO se pone en is_active=false (no es baja lógica).
- El guard de duplicados es a nivel aplicación: impide cargar dos veces el mismo proyecto/categoría con una línea activa cuya aprobación no esté RECHAZADO; tras un rechazo permite la re-carga.
- Las horas rechazadas no cuentan para nómina. Horas efectivas: APROBADO → cargadas; APROBADO_CON_OBSERVACION → sugeridas; PENDIENTE/RECHAZADO → 0.
