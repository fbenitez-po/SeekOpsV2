# Plan: Migración de datos seekops_legacy → seekops

## Contexto

Dos bases de datos PostgreSQL en el mismo servidor:
- **seekops_legacy**: Django app legacy, 44 tablas, ~75k filas
- **seekops** (nueva): schema normalizado custom, 31 tablas

La migración usa la extensión `dblink` para leer `seekops_legacy` directamente desde `seekops`. Todos los scripts son idempotentes: los pasos 2-4 vía `ON CONFLICT DO NOTHING` (tienen unique natural), y los pasos 5, 6 y 7 vía `DELETE FROM <tabla> WHERE created_by='migration'` al inicio del paso (esas tablas no tienen unique natural, así que el `ON CONFLICT` solo no bastaba y re-correr duplicaba).
...
|-----------------|------|------|--------|--------|-------|
| `areas` | Catálogo | 1 | seeds.sql | ✅ | 8 |
| `teams` | Catálogo | 1 | seeds.sql | ✅ | 13 |
| `client_sectors` | Catálogo | 1 | seeds.sql | ✅ | 33 |
| `client_segmentations` | Catálogo | 1 | seeds.sql | ✅ | 7 |
| `productivity_layers` | Catálogo | 1 | seeds.sql | ✅ | 3 |
| `work_categories` | Catálogo | 1 | seeds.sql | ✅ | 18 |
| `project_categories` | Catálogo | 1 | seeds.sql | ✅ | 34 |
| `project_segmentation` | Catálogo | 1 | seeds.sql | ✅ | 9 |
| `income_categories` | Catálogo | 1 | seeds.sql | ✅ | 5 |
| `profiles` | Catálogo | 1 | seeds.sql | ✅ | 3 |
| `document_types` | Catálogo | 1 | seeds.sql | ✅ | 7 |
| `service_types` | Catálogo | 1 | seeds.sql | ✅ | 2 |
| `periods` | Catálogo | 1 | seeds.sql | ✅ | pre-cargado Jan 2026→May 2028 |
| `users` | Entidad | 2 | legacy-migration.sql | ✅ | 275 |
| `user_area` | Relación (M2M) | 2 | legacy-migration.sql | ✅ | incluido en paso 2 |
| `user_profile` | Relación (M2M) | 2 | legacy-migration.sql | ✅ | incluido en paso 2 |
| `clients` | Entidad | 3 | legacy-migration.sql | ✅ | 102 |
| `projects` | Entidad | 4 | legacy-migration.sql | ✅ | 490 |
| `project_project_category` | Relación (M2M) | 4 | legacy-migration.sql | ✅ | 372 |
| `project_user` | Relación (M2M) | 4 | legacy-migration.sql | ✅ | 1090 (role='SEEKER') |
| `hour_projections` | Transaccional | 5 | legacy-migration.sql | ✅ | 2.563 (1.965 activas, 598 anuladas) |
| `commercial_records` | Transaccional | 6 | legacy-migration.sql | ✅ | 677 (1 excluido: precio negativo) |
| `time_entries` | Transaccional | 7 | legacy-migration.sql | ✅ | 1.593 (sin status: el estado vive en time_entry_approvals) |
| `time_entry_lines` | Transaccional | 7 | legacy-migration.sql | ✅ | 2.928 activas (horas originales del seeker; sin status) |
| `time_entry_approvals` | Fuente de verdad de estado | 7 | legacy-migration.sql | ✅ | 2.928 (una por línea; PENDIENTE/APROBADO/APROBADO_CON_OBSERVACION/RECHAZADO) |
...
**Qué hace:** Carga los datos de referencia que el resto de las tablas necesitan como FK. Son tablas que no vienen de `seekops_legacy` sino que se re-crean desde cero en `seeds.sql` con los mismos valores del sistema anterior.

**Tablas populadas en seekops:**

| Tabla en seekops | Equivalente en seekops_old | Filas | Cómo se resuelve |
|------------------|---------------------------|-------|-----------------|
| `areas` | `masters_area` | 8 | Hardcoded en seeds.sql — mismos nombres |
| `teams` | `masters_team` | 13 | Hardcoded en seeds.sql — mismos nombres |
| `client_sectors` | `masters_sector` | 33 | Hardcoded en seeds.sql — mismos nombres |
| `client_segmentations` | `masters_segmentation` | 7 | Hardcoded en seeds.sql — mismos nombres |
| `productivity_layers` | `masters_layerproductivity` | 3 | Hardcoded en seeds.sql — mismos nombres |
| `work_categories` | `masters_usercategory` | 18 | Hardcoded en seeds.sql — mismos nombres |
| `project_categories` | `masters_extensioncategory` | 34 | Hardcoded en seeds.sql — mismos nombres |
| `project_segmentation` | `masters_division` | 9 | Hardcoded en seeds.sql — mismos nombres |
...
**Script:** `legacy-migration.sql` (paso 2)
...
**Script:** `legacy-migration.sql` (paso 3)
...
**Script:** `legacy-migration.sql` (paso 4)
...
**Script:** `legacy-migration.sql` (paso 5)
...
**Script:** `legacy-migration.sql` (paso 6)
...
**Script:** `legacy-migration.sql` (paso 7)
