...
const formVacio = { proyecto_id: '', periodo_id: '', monto: '' };

export default function IngresosCrear() {
  ...
  const mutCrear = useMutation({
    mutationFn: (datos) => ingresosApi.crear(datos),
    onSuccess: () => {
  ...
  const valido = form.proyecto_id && form.periodo_id && form.monto !== '';
  const error = mutCrear.error?.response?.data?.error || mutCrearYAnadir.error?.response?.data?.error;

  function datos() {
    return { proyecto_id: form.proyecto_id, periodo_id: form.periodo_id, monto: Number(form.monto) };
  }
  ...
  return (
    <Layout>
      <div className="space-y-6">
        <div>
          <button type="button" onClick={() => navigate('/admin/ingresos')} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors mb-3">
            <ArrowLeft className="h-4 w-4" /> Volver a Ingresos
          </button>
          <h1 className="text-2xl font-bold">Añadir Ingreso</h1>
        </div>
        ...
        <Card className="max-w-lg">
          <CardContent className="p-6">
            <div className="grid grid-cols-[auto_1fr] gap-x-6 gap-y-4 items-center">
              ...
              <label className="text-sm font-medium">Ingreso:</label>
              <Input
                type="number"
                min="0"
                step="0.01"
            ...
            <div className="flex gap-2 mt-6">
              <Button
                variant="outline"
                onClick={() => mutCrearYAnadir.mutate(datos())}
                disabled={!valido || mutCrearYAnadir.isPending}
              >
                Guardar y añadir otro
              </Button>
              <Button
                onClick={() => mutCrear.mutate(datos())}
                disabled={!valido || mutCrear.isPending}
              >
                Guardar
              </Button>
            </div>
