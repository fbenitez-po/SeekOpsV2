...
export default function GastosAdminCrear() {
  ...
  const mutCrear = useMutation({
    mutationFn: (datos) => gastosAdminApi.crear(datos),
  ...
  const mutCrearYAnadir = useMutation({
    mutationFn: (datos) => gastosAdminApi.crear(datos),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['gastos-admin'] }); setForm((f) => ({ ...f, codigo: '', descripcion: '', monto: '' })); },
  });
    ...
    <Layout>
      <div className="space-y-6">
        <div>
          ...
          <h1 className="text-2xl font-bold">Añadir Gasto Administrativo</h1>
        </div>
