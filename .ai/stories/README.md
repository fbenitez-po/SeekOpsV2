# Historias de Usuario — Seekops

> Índice de todas las historias del proyecto, organizadas por rol/epic.
> **Estado:** 21 historias aprobadas ✅

---

## Epic 00 — General: Funcionalidades Transversales

| ID | Título | Estado |
|---|---|---|
| US-100 | Login | ✅ |
| US-101 | Reset/Recuperar contraseña | ✅ |
| US-102 | Home Seeker | ✅ |
| US-103 | Home Gestor | ✅ |
| US-104 | Home Admin | ✅ |

Ver: [`epic-00-general.md`](epic-00-general.md)

---

## Epic 01 — Seeker: Carga y Seguimiento de Horas

| ID | Título | Estado |
|---|---|---|
| US-001 | Registrar horas semanales en un proyecto | ✅ |
| US-002 | Visualizar historial de horas cargadas | ✅ |
| US-003 | Ajustar horas observadas | ✅ |

Ver: [`epic-01-seeker.md`](epic-01-seeker.md)

---

## Epic 02 — Gestor: Aprobación y Observaciones

| ID | Título | Estado |
|---|---|---|
| US-004 | Ver horas pendientes de aprobación del equipo | ✅ |
| US-005 | Aprobar horas de un seeker | ✅ |
| US-006 | Observar horas y solicitar corrección | ✅ |
| US-007 | Rechazar horas | ✅ |
| US-008 | Registrar mis propias horas como Gestor | ✅ |

Ver: [`epic-02-gestor.md`](epic-02-gestor.md)

---

## Epic 03 — Admin: Gestión de Sistema

| ID | Título | Estado |
|---|---|---|
| US-009 | Crear usuario | ✅ |
| US-010 | Crear cliente | ✅ |
| US-011 | Crear proyecto | ✅ |
| US-012 | Asignar usuarios a proyectos con rol | ✅ |
| US-013 | Editar usuario | ✅ |
| US-014 | Editar cliente | ✅ |
| US-015 | Editar proyecto | ✅ |
| US-016 | Administración de accesos y roles | ✅ |

Ver: [`epic-03-admin.md`](epic-03-admin.md)

---

## Decisiones clave documentadas

✅ **Notificaciones:** Por email  
✅ **Auditoria:** Quién cambió qué y cuándo  
✅ **Período retroactivo:** Sin límite  
✅ **Múltiples roles:** Un usuario puede ser Seeker en un proyecto y Gestor en otro (incluso ambos en el mismo)  
✅ **Auto-aprobación:** El Gestor se auto-aprueba cuando carga horas en su propio proyecto  
✅ **Horas rechazadas:** Se visualizan sin opción de editar (definitivas)

---

## Próximo paso

Cuando todas las historias estén aprobadas → **UX Flows** (`/ux-user-flow`)

