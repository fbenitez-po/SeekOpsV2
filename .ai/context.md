...
- **Últimas actualizaciones:**
  - ✅ **Approvals como fuente de verdad única (2026-05-26):** Eliminación del estado de semana y del estado por línea. `time_entries` y `time_entry_lines` ya no tienen columna `status`. `time_entry_approvals` es la única fuente de verdad: una fila por línea (`time_entry_line_id` UNIQUE), creada en `PENDIENTE` al cargar el seeker, mutada por el gestor a `APROBADO | APROBADO_CON_OBSERVACION | RECHAZADO`. Al aprobar con observación, las horas sugeridas van solo a `suggested_hours`/`suggested_extra_hours` — las `time_entry_lines.hours` nunca se modifican. El badge de estado de semana fue eliminado de toda la UI; cada pantalla muestra el bloque de semana con proyectos+estados desplegables. Horas efectivas: `APROBADO_CON_OBSERVACION` → sugeridas; `APROBADO` → cargadas; `PENDIENTE`/`RECHAZADO` → 0 (no cuentan en totales). `linea_id` reemplaza a `categoria_ingreso_id` en los payloads de approve/observe/reject. Migración: `approvals_as_source_of_truth`.
  ...
  - ✅ US-004 (nueva): Alerta de semanas sin carga en Home Seeker — muestra al ingresar las semanas desde `fecha_ingreso` hasta hoy que no tienen ninguna carga registrada. Permanente hasta que se carguen. Lógica en JS en el servicio, iterando por fecha de inicio de semana (lunes) y comparando contra los `week_start_date` cargados.
...
- **Módulo `dashboard` — API de integración externa (2026-05-27):** Nuevo módulo paraguas `backend/src/modules/dashboard/` que migra la "API Dashboard" de v1 (endpoints read-only, abiertos y sin paginación que alimentan BI). Sigue el patrón de sub-módulos de `finance/`: `dashboard.routes.ts` monta sub-routers por recurso. Primer recurso migrado: **`GET /api/dashboard/seekers`** (= `User.objects.all()` de v1). Decisiones clave: (1) **Endpoint abierto** — sin `verifyToken`/`adminOnly`, como v1; el *hook de auth futuro* (token compartido estilo `require_token(?token=...)` de v1) se monta como un único `router.use(...)` en `dashboard.routes.ts` sin tocar los sub-recursos. (2) **Array plano** sin envoltorio `{ data, pagination }`. (3) **Excepción al contrato en español:** este módulo expone **claves en inglés** heredadas de v1 (`email, first_name, last_name, job, cellphone, document_number, team, is_active`), a diferencia del resto de la API v2 que usa claves en español. Es una excepción consciente al spec `api-contract` para preservar compatibilidad byte-a-byte con el consumidor BI; la traducción v2→v1 vive en el `mapper` del sub-recurso (`position→job`, `mobile_phone→cellphone`, `teams.name→team`). (4) **Repository propio read-only** que no reusa el `service`/`mapper` del módulo `users` (aislamiento de contrato). **Módulo `dashboard` completo (2026-05-27):** los 4 recursos de la "API Dashboard" de v1 migrados (`seekers`, `clients`, `commercial`, `project`), todos abiertos, read-only, array plano. **`GET /api/dashboard/project` migrado**: array plano desnormalizado con claves `__` de v1; traduce `actual_start_date→real_start_date`, `actual_end_date→real_end_date`; desnormaliza cliente/manager/`productivity_layers.name`; **gaps de v2 en `null`** (status, tier, evaluation_internal/external, image, flag_poll, comments_date, category__iframe_poll) más las categorías `category__name`/`category_extension__name` (Q2/Q3 diferidas, sin confirmar mapeo v1↔v2). **`GET /api/dashboard/commercial` migrado (2026-05-27)**: array plano desnormalizado con claves `__` de v1; traduce `record_date→date`, `currency→coin`, `has_contract→status`, `is_billed→billing`, `owner→responsible__*`, `document_types.name→document`; gaps en `null` (`type`, `division__name` —v2 no tiene divisions—, `duration`); `project__category__name` = primera categoría M2M o null. **`GET /api/dashboard/clients` migrado (2026-05-27)**: array plano con claves de v1 (`id, business_reason=legal_name, business_name=trade_name, business_number=ruc, fiscal_address=address, legal_address=null, segmentation=segmentation_id, sector=sector_id`); decisiones del change: `id`/`segmentation`/`sector` como UUID v2, y las dos direcciones de v1 cubiertas por el único `address` (`fiscal_address`=address, `legal_address`=null).
- **Semana como rango de fechas (2026-05-25):** Se eliminó el código string `"S21/26"` (semana N del año) como identidad temporal de `time_entries`. La columna `week VARCHAR(10)` se reemplazó por `week_start_date DATE` (lunes) + `week_end_date DATE` (domingo), con `UNIQUE(user_id, week_start_date)` e índices `idx_time_entries_week_start` / `idx_time_entries_user_week_start`. Motivo: existían **tres algoritmos divergentes** para mapear fecha↔código (`calcularCodigoSemana`/`formatearSemana` calendario, `semanaADomingo`, y el parse SQL `to_date(...'IYYYIW')` ISO en projections/personnelCosts) que discrepaban en los límites de año. Cambios: (1) API — `time_entries` expone `semana_inicio`/`semana_fin` (ISO `YYYY-MM-DD`); el POST recibe ambos (valida lunes y fin = inicio+6); filtro `?semana_inicio=`. (2) `semanas-sin-carga` devuelve `{ semanas: [{semana_inicio, semana_fin}], total }`. (3) Las queries SQL de alertas (`projections.repository`) y de horas usadas (`personnelCosts.repository`) ya no parsean strings: usan comparación directa de rangos / `EXTRACT` sobre `week_start_date`. (4) Frontend — se eliminaron `formatearSemana`/`semanaADomingo`; nuevo helper `rangoSemana(inicioISO)`; la UI muestra **solo el rango** ("Lun 18 al Dom 24 may 2026"), sin número de semana. (5) Migración limpia (`migrate dev` + `reset`), sin backfill (dev).
