...
async function enviar(opciones) {
  ...
  if (!transporter) {
    console.log(`[EMAIL - DEV] Para: ${opciones.to}`);
    console.log(`[EMAIL - DEV] Asunto: ${opciones.subject}`);
    console.log(`[EMAIL - DEV] Link: ${opciones.link}`);
    return;
  }
...
async function enviarBienvenida(email, nombres, token) {
  ...
  await enviar({
    ...
    html: templateBase('Activá tu cuenta', cuerpo),
    link,
  });
}

async function enviarResetPassword(email, nombres, token) {
  ...
  await enviar({
    ...
    html: templateBase('Restablecer contraseña', cuerpo),
    link,
  });
}
