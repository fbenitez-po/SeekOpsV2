...
import * as service from './salesCosts.service';
import type { ListSalesCostsQuery } from './salesCosts.schema';

export async function list(req: Request, res: Response, next: NextFunction) {
  try {
    res.json(await service.list(req.query as unknown as ListSalesCostsQuery));
  } catch (err) {
    next(err);
  }
}
