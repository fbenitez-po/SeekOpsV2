...
router.get('/:id', asyncHandler(async (req, res) => {
  ...
  if (!item) { res.status(404).json({ error: 'Costo de venta no encontrado' }); return; }
  res.json(item);
}));
...
router.put('/:id', asyncHandler(async (req, res) => {
  ...
  if (!updated) { res.status(404).json({ error: 'Costo de venta no encontrado' }); return; }
  res.json(updated);
}));

router.delete('/:id', asyncHandler(async (req, res) => {
  const ok = await repo.remove(req.params['id'] as string);
  if (!ok) { res.status(404).json({ error: 'Costo de venta no encontrado' }); return; }
  res.json({ ok: true });
}));
