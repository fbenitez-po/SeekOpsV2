...
import type { ListCommercialQuery } from './commercial.schema';

export async function list(req: Request, res: Response, next: NextFunction) {
  try {
    res.json(await service.list(req.query as unknown as ListCommercialQuery));
  } catch (err) {
    next(err);
  }
}
...
export async function documentTypes(_req: Request, res: Response, next: NextFunction) {
  try {
    res.json(await service.documentTypes());
  } catch (err) {
    next(err);
  }
}
